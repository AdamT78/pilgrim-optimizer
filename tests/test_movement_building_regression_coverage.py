...
def test_combined_kogge_cloisters_action_id_includes_both_sources() -> None:
    ...
    assert ":sow_route_building:kogge:from:market" in combined_id
    assert ":secondary_building:cloisters:secondary_from:market" in combined_id
...
def test_apply_rejects_hired_cloisters_when_hire_source_is_unavailable() -> None:
    hire_scenario = load_scenario("scenarios/cloisters_hire_market_skip_duty_tile_001.json")
    hired_action = _first_action(
        legal_actions(hire_scenario.state, hire_scenario.config),
        lambda candidate: candidate.sow_route_building_source == "market",
    )
