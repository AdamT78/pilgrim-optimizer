...
class _SowRouteOption:
    ...
    route: tuple[int, ...]
    # Enumeration spends a route hire only in this private branch state.  The action it produces
    # still records the source and payment and does not move stock until Confirm applies it.
    state: GameState
...
class _HirePaymentOption:
    ...
    On a plain Merchant resource there is one option; on the cornucopia there is one option per
    affordable resource the payer may choose between.
...
def _legal_building_conversion_options(
    ...
    return tuple(option for generator in generators for option in generator(state, config))
...
def apply_turn_step(
    ...
    if isinstance(step, BuildingRelocationStep):
        return _apply_building_relocation_step(state, config, step)
...
def _apply_building_activation_step(
    ...
    if (
        step.building_id
        in (_BUILDING_PULPIT, *_HIRED_MODIFIER_BUILDING_IDS)
    ...
    if step.building_id in _HIRED_MODIFIER_BUILDING_IDS and not _is_hired_source(source):
        raise TransitionValidationError(
    ...
    if step.building_id in _HIRED_MODIFIER_BUILDING_IDS:
        return replace(
            next_state,
...
def _apply_building_relocation_step(
    state: GameState,
    config: GameConfig,
    step: BuildingRelocationStep,
) -> GameState:
    if step.building_id == "library":
        return _apply_library_relocation_step(state, config, step)
    if state.turn_progress.resolution_committed:
        raise TransitionValidationError(
            "Dormitory and Inquisition relocation steps are only available before resolution."
        )
    if step.building_id in state.turn_progress.used_buildings:
        raise TransitionValidationError(f"Building already used this turn: {step.building_id}.")
    if step.building_id not in {"dormitory", "inquisition"}:
        raise TransitionValidationError(
            f"Unsupported building relocation step: {step.building_id}."
        )
    if not isinstance(step.selected_position, int):
        raise TransitionValidationError(
            "Dormitory and Inquisition relocation targets must be positions."
        )
    ...
    player_vector = next_state.player_vector(player)
    if step.selected_position not in duty_positions:
        raise TransitionValidationError(
            f"{step.building_id.title()} relocation target must be a Duty position."
        )
    if step.building_id == "dormitory":
        from_position, to_position = step.selected_position, city_position
        if player_vector[from_position] <= 0:
            raise TransitionValidationError(
                "Dormitory relocation source must contain at least one acting-player acolyte."
            )
    else:
        from_position, to_position = city_position, step.selected_position
        if player_vector[from_position] <= 0:
            raise TransitionValidationError(
    ...
    try:
        ...
        raise TransitionValidationError(str(exc)) from exc
    next_state = next_state.with_player_vector(player, relocated_vector)
    events.append(
...
def _apply_library_relocation_step(
    state: GameState,
    config: GameConfig,
    step: BuildingRelocationStep,
) -> GameState:
    if not state.turn_progress.resolution_committed:
        raise TransitionValidationError(
            "Library relocation steps are only available after resolution."
        )
    if step.building_id in state.turn_progress.used_buildings:
        raise TransitionValidationError(f"Building already used this turn: {step.building_id}.")
    ...
    if _is_hired_source(source):
        if step.hire_payment not in CORNUCOPIA_HIRE_RESOURCES:
            raise TransitionValidationError("Library hire payment must name a concrete resource.")
        if source.hire_resource == CORNUCOPIA_COUNTER:
            source = replace(source, hire_resource=step.hire_payment, hire_resource_chosen=True)
        elif source.hire_resource != step.hire_payment:
            raise TransitionValidationError(
                "Library hire payment does not match the Merchant's current counter."
            )
        try:
            next_state, payment = apply_building_hire_payment(
                next_state,
        ...
        raise TransitionValidationError("Own-active Library does not take a hire payment.")

    city_position = config.board.index_for_name("city")
    selected_position = step.selected_position
    if isinstance(selected_position, str):
        selected = selected_position.strip().lower().replace("-", "_").replace(" ", "_")
        if selected != _LIBRARY_ABBEY_TARGET:
            raise TransitionValidationError(
        ...
        isinstance(selected_position, int)
        and not isinstance(selected_position, bool)
        and selected_position in set(config.duty_positions())
        and selected_position != city_position
    ):
        to_position = selected_position
        to_pool = config.board.positions[selected_position]
    else:
        raise TransitionValidationError(
            "Library relocation target must be Abbey or a non-city Duty tile."
        )

    if next_state.player_vector(player)[city_position] <= 0:
        raise TransitionValidationError(
    ...
    next_state = _apply_end_turn_relocation_to_state(
        next_state,
        player=player,
...
def turn_step_id(step: TurnStep) -> str:
    ...
    if isinstance(step, BuildingRelocationStep):
        payment = f":pay:{step.hire_payment}" if step.hire_payment is not None else ""
        return (
            f"turn_step:building_relocation:{step.building_id}:from:{step.source}"
            f":selected:{step.selected_position}{payment}"
        )
    payment = f":pay:{step.hire_payment}" if step.hire_payment is not None else ""
    return (
    ...
    *,
    ...
    for origin in occupied_positions(player_vector):
        ...
        for route_option in _legal_sow_routes_for_origin(
            ...
            route_state = route_option.state
            ...
            for duty_position in config.duty_positions():
                ...
                if TurnResolutionType.GIVE_ALMS_PAID in category_actions:
                    ...
                    bank_sequence_seed = _player_state_with_wheat_delta(
                        player_state,
                        wheat_delta=((2 if no_hire_mill_active else 0) + player_resources.silver),
                    )
                    if (
...
def _apply_full_turn_action(
    ...
    route_hired_sources = tuple(
        source
        for source in (
            kogge_source,
            cloisters_route.source if cloisters_route is not None else None,
        )
        if source is not None and _is_hired_source(source)
    )
    route_hired_buildings = frozenset(source.building_key for source in route_hired_sources)
    for route_hire_source in route_hired_sources:
        try:
            state_for_sow, hire_payment = apply_building_hire_payment(
                state_for_sow,
                acting_player=player,
                source=route_hire_source,
            )
        except ValueError as exc:
            raise TransitionValidationError(str(exc)) from exc
        pre_sowing_events.append(
    ...
    if action.resolution is TurnResolutionType.TITHE:
        ...
        if action.resolution is not TurnResolutionType.GIVE_ALMS_PAID and (
            action.alms_payment_silver != 0 or action.alms_payment_wheat != 0
        ):
            raise TransitionValidationError(
        ...
        if action.sow_route_building_source is not None and not has_route_building_id:
            raise TransitionValidationError(
                "sow_route_building_source requires sow_route_building_id."
            )
        if (
            action.sow_route_secondary_building_source is not None
            and not has_secondary_route_building_id
        ):
            raise TransitionValidationError(
                "sow_route_secondary_building_source requires sow_route_secondary_building_id."
            )
        ...
        _validate_hire_payments_for_action(
            action,
            hired_buildings=hire_context.hired_buildings,
        )
            ...
            if action.resolution in (
                ...
                if action.resolution is TurnResolutionType.PRODUCE_WHEAT:
                    ...
                    combined_wheat_amounts = ",".join(
                        str(amount) for amount in (wheat_bonus, well_bonus) if amount > 0
                    )
                    if wheat_bonus:
                    ...
                    combined_stone_sources = ",".join(
                        ...
                        for source, amount in (
                            ("stone_mason", stone_bonus),
                            ("quarry", quarry_bonus),
                        )
                        if amount > 0
                    )
                    combined_stone_amounts = ",".join(
                        str(amount) for amount in (stone_bonus, quarry_bonus) if amount > 0
                    )
                    if stone_bonus:
...
def _legal_start_turn_relocation_steps(
    state: GameState,
    config: GameConfig,
) -> tuple[BuildingRelocationStep, ...]:
    return (
        *_legal_dormitory_relocation_steps(state, config),
        *_legal_inquisition_relocation_steps(state, config),
    )
...
def _legal_dormitory_relocation_steps(
    ...
    return tuple(
        ...
        for duty_position in config.duty_positions()
        if state_after_hire.player_vector(state.active_player)[duty_position] > 0
    )


def _legal_inquisition_relocation_steps(
    state: GameState,
    config: GameConfig,
) -> tuple[BuildingRelocationStep, ...]:
...
def _legal_library_relocation_steps(
    state: GameState,
    config: GameConfig,
) -> tuple[BuildingRelocationStep, ...]:
    source = _activatable_building_source(state, config, building_key="library")
...
def _legal_sow_routes_for_origin(
    ...
    *,
    ...
    for kogge_source, state_after_kogge_hire in _route_building_source_variants(
        state,
        config,
    ...
    routes.extend(
        _SowRouteOption(route=route, state=state)
...
def _route_building_source_variants(
    state: GameState,
    config: GameConfig,
    *,
    building_id: str,
) -> tuple[tuple[BuildingAbilitySource, GameState], ...]:
    """Return action-carried route sources with the stock each candidate has left to spend."""
    source = building_ability_source(
        state,
        config,
        acting_player=state.active_player,
...
def _legal_cloisters_route_options(
    ...
    *,
    ...
    return tuple(
        _SowRouteOption(
            route=variant.route,
            state=state_after_cloisters_hire,
            building_id=_ROUTE_BUILDING_CLOISTERS,
            source=cloisters_source,
            omitted_location=variant.omitted_location,
        )
        for cloisters_source, state_after_cloisters_hire in _route_building_source_variants(
            state,
            config,
            building_id=_ROUTE_BUILDING_CLOISTERS,
        )
    ...
    *,
    ...
    if source.source_type == "own_active" or _is_hired_source(source):
        action = replace(
            ...
            sow_route_building_source=_own_or_hired_building_source_label(source),
        )
        return _with_hire_payment_for_source(action, source=source)
    ...
    *,
    ...
    if source.source_type == "own_active" or _is_hired_source(source):
        action = replace(
            ...
            sow_route_secondary_building_source=_own_or_hired_building_source_label(source),
            sow_route_omitted_location=omitted_location,
        )
        return _with_hire_payment_for_source(action, source=source)
...
def _legal_hire_payment_options(
    *,
    ...
    Usually one: the Merchant offers a single resource and the payer takes it or cannot afford it.
    On the cornucopia it is one per resource the payer can actually afford, because the wildcard
    lets them choose. If they can afford none, there are no options and the hire is unavailable,
    which is the same answer Taxation gives for a different reason.

    Only affordable resources are listed. Offering a variant that cannot be paid would be a legal
    action that immediately fails.
    ...
    if player_state is None:
        raise TransitionValidationError(
            "A wildcard hire cannot be enumerated without the payer's stock; the affordable "
            "resources are what the choice is drawn from."
        )
    return tuple(
        _HirePaymentOption(
            resource=candidate,
            source=replace(source, hire_resource=candidate, hire_resource_chosen=True),
            chosen=True,
        )
        for candidate in CORNUCOPIA_HIRE_RESOURCES
        if _resource_amount_of(player_state, candidate) >= source.hire_cost
    )


def _resource_amount_of(player_state: PlayerState, resource: str) -> int:
    resources = player_state.resources
    if resource == "stone":
    ...
    if resource == "piety":
        return resources.piety
    raise TransitionValidationError(f"Unknown hire payment resource: {resource}.")


def _hire_source_for_action(
    source: BuildingAbilitySource,
    action: FullTurnAction,
) -> BuildingAbilitySource:
    """Resolve one hire source to the resource this action recorded for that building."""
    if not _is_hired_source(source):
        return source
    payments = dict(action.hire_payments)
    chosen = payments.get(source.building_key)
    if chosen is None:
        if _action_declares_hire_for_building(action, source.building_key):
            raise TransitionValidationError(
                f"Missing hire payment for {source.building_key}; action must record one per hire."
            )
        return source
    if chosen not in CORNUCOPIA_HIRE_RESOURCES:
        raise TransitionValidationError(
            "hire_payments values must be one of "
            f"{', '.join(CORNUCOPIA_HIRE_RESOURCES)}; got {chosen!r} for {source.building_key}."
        )
    if source.hire_resource == CORNUCOPIA_COUNTER:
        return replace(source, hire_resource=chosen, hire_resource_chosen=True)
    if source.hire_resource != chosen:
        raise TransitionValidationError(
            f"hire_payments recorded {source.building_key}={chosen}, "
            f"but the resolved source requires {source.hire_resource}."
        )
    return source
...
def _hire_payment_source_variants(
    ...
    return tuple(
        option.source
        for option in _legal_hire_payment_options(source=source, player_state=player_state)
    )
...
def _hire_payment_states(
    ...
    for hire_source in _hire_payment_source_variants(
        source,
        state.player_state(state.active_player),
    ):
...
def _with_hire_payment_fields(
    ...
    *,
    ...
    """Stamp one hire payment onto a constructed action."""
    return _with_hire_payment_for_source(action, source=option.source)


def _with_hire_payment_for_source(
    action: FullTurnAction,
    *,
    source: BuildingAbilitySource,
) -> FullTurnAction:
    ...
    if resource is None or resource == CORNUCOPIA_COUNTER:
        raise ValueError(
            f"Hire source for {source.building_key} is not resolved to a concrete payment resource."
        )
    if resource not in CORNUCOPIA_HIRE_RESOURCES:
        allowed = ", ".join(CORNUCOPIA_HIRE_RESOURCES)
        raise ValueError(
            f"Hire source for {source.building_key} has unsupported resource {resource!r}; "
            f"expected one of {allowed}."
        )
    current = dict(action.hire_payments)
    previous = current.get(source.building_key)
    if previous is not None and previous != resource:
        raise ValueError(
            "Action names two different payments for one hired building "
            f"({source.building_key}: {previous} and {resource})."
        )
    current[source.building_key] = resource
    canonical = tuple(sorted(current.items()))
    return replace(action, hire_payments=canonical)


def _declared_hired_buildings(action: FullTurnAction) -> tuple[str, ...]:
    ...
    return tuple(
        building_id
        for building_id, source_label in candidates
...
def _action_declares_hire_for_building(action: FullTurnAction, building_key: str) -> bool:
    return building_key in _declared_hired_buildings(action)


def _validate_hire_payments_for_action(
    action: FullTurnAction,
    *,
    hired_buildings: tuple[str, ...],
) -> None:
    entries = action.hire_payments
    if tuple(sorted(entries)) != entries:
        raise TransitionValidationError("hire_payments must be sorted by building id.")
    seen: set[str] = set()
    for pair in entries:
        if not isinstance(pair, tuple) or len(pair) != 2:
            raise TransitionValidationError(
                "hire_payments must be a tuple of (building_id, resource) pairs."
            )
        building_id, resource = pair
        if not isinstance(building_id, str) or not building_id:
            raise TransitionValidationError("hire_payments building ids must be non-empty strings.")
        if building_id in seen:
            raise TransitionValidationError(
                "hire_payments cannot repeat a building id; one hire payment key is one building."
            )
        if resource not in CORNUCOPIA_HIRE_RESOURCES:
            allowed = ", ".join(CORNUCOPIA_HIRE_RESOURCES)
            raise TransitionValidationError(
                f"hire_payments resource for {building_id} must be one of {allowed}; got {resource!r}."
            )
        seen.add(building_id)

    duplicates = sorted(
        building for building, count in Counter(hired_buildings).items() if count > 1
    )
    if duplicates:
        listed = ", ".join(duplicates)
        raise TransitionValidationError(
            "Action names the same hired building in multiple slots, which is ambiguous for "
            f"hire_payments: {listed}."
        )

    hired_set = set(hired_buildings)
    missing = sorted(hired_set - seen)
    extras = sorted(seen - hired_set)
    if missing or extras:
        messages: list[str] = []
        if missing:
            messages.append("missing payments for " + ", ".join(missing))
        if extras:
            messages.append("payments for non-hired buildings " + ", ".join(extras))
        message = "hire_payments must match hired sources exactly: " + "; ".join(messages) + "."
        raise TransitionValidationError(message)
...
def _resolved_cloisters_route_for_action(
    *,
    ...
    expected_source_label = _own_or_hired_building_source_label(source)
    if source_label != expected_source_label:
        raise TransitionValidationError(
            "Cloisters route source does not match the action's available building source: "
            f"expected {expected_source_label}."
        )
...
def _resolved_kogge_source_for_action(
    *,
    ...
    if not action_has_kogge_fields:
        raise TransitionValidationError("Kogge route must include its sow-route building id.")
    expected_source_label = _own_or_hired_building_source_label(source)
    if source_label != expected_source_label:
        raise TransitionValidationError(
            "Kogge route source does not match the action's available building source: "
            f"expected {expected_source_label}."
        )
    return source
...
def _start_turn_building_bonus_event(
    *,
    ...
    from_name = config.board.positions[from_position]
    to_name = config.board.positions[to_position]
    return GameEvent(
        event_type=EventType.BUILDING_BONUS,
        actor=actor,
        action_id=action_id,
