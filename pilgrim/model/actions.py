...
TurnStep = BuildingConversionStep | BuildingActivationStep | BuildingRelocationStep
...
def action_id(action: GameAction) -> str:
    ...
    if (
        ...
        sow_route_suffix = f":sow_route_building:{action.sow_route_building_id or 'none'}"
        if action.sow_route_building_source is not None:
            sow_route_suffix += f":from:{action.sow_route_building_source}"
        if (
            action.sow_route_secondary_building_id is not None
            or action.sow_route_secondary_building_source is not None
        ):
            sow_route_suffix += f":secondary_building:{action.sow_route_secondary_building_id or 'none'}"
            if action.sow_route_secondary_building_source is not None:
                sow_route_suffix += f":secondary_from:{action.sow_route_secondary_building_source}"
...
def _ordination_cost_phrase(
    ...
    has_mill = (
        "mill" in _active_buildings_for_actor(state, actor) or action.hired_building_id == "mill"
    )
...
def action_choice_summary_for_players(
    ...
    *,
    ...
    if isinstance(action, SetupSowAction):
        ...
        ending = _board_label_for_position(
            config, action.route[-1] if action.route else action.origin
        )
...
def action_summary(action: GameAction, config: GameConfig) -> str:
    ...
    if isinstance(action, StartPlayerSelectionAction):
        return (
            f"Start player selection: {action.chosen_start_player.name.lower()} begins this round"
        )
    ...
    elif uses_cloisters and action.sow_route_omitted_location is not None:
        route_summary += (
