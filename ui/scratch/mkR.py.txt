...
SP="/tmp/claude-0/-home-claude/d6f24f2e-391f-5b12-98d7-6b8efe6506fb/scratchpad/"
OUT="/home/claude/ui-2.0/"
sys.path.insert(0,'tools'); sys.path.insert(0,'.'); sys.path.insert(0,SP); sys.path.insert(0,OUT)
spec=importlib.util.spec_from_file_location("gb", SP+"gen_board_local.py")
gb=importlib.util.module_from_spec(spec)
with contextlib.redirect_stdout(io.StringIO()): spec.loader.exec_module(gb)
...
import re as _re
wheel=pathlib.Path('/tmp/wheel1.svg').read_text()
...
_HORN = _json.load(open('tools/ui_debug/duty_wheel_layout.json'))['artwork']['cornucopia_horn_path']
...
_WAGON_D = pathlib.Path(OUT + "wagon_path_nospokes.txt").read_text().strip()
...
_mkt=pathlib.Path('/tmp/market.html').read_text()
def _tiles(html):
    out=[]; i=0
    while True:
...
# Give Alms west.
_ARROW_D = _json.load(open('tools/ui_debug/duty_wheel_layout.json'))['artwork']['middle_arrow_path']
...
_RING_D = _json.load(open('tools/ui_debug/duty_wheel_layout.json'))['artwork']['ring_arrow_path']
...
_map_svg = pathlib.Path('/tmp/map_populated.svg').read_text()
...
_mktsvg = pathlib.Path('/tmp/market_hex.svg').read_text()
...
_logbody = pathlib.Path('/tmp/log_disc.html').read_text()
...
pathlib.Path('/tmp/log_final.html').write_text(_WGN_DEFS + _logbody)
